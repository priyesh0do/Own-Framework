...
public class ResidentialFlashReport {

	ExtentTest test;
	WebDriver driver;

	public ResidentialFlashReport(WebDriver driver, ExtentTest test) {
		this.driver = driver;
		this.test = test;
		PageFactory.initElements(driver, this);
	}

	@FindBy(xpath = "//*[@id='jsFlashLink']/span[contains(., 'See Residential Flash Report')]")
	WebElement ResidentialFlashReportLink;
	...
	@FindBy(xpath ="//*[@id='actExportPDF']/span[@id='lblExcelExport']")
	WebElement PDFLink;


	public void clickResidentialFlashReport() throws IOException {
		try
		{
		Util.ElementExist(driver, ResidentialFlashReportLink);
		ResidentialFlashReportLink.click();
		Report.PassTest(test, "Clicked on Residential Flash Report");
		}
		catch(Exception e)
		{
			Report.FailTestSnapshot(test, driver, "Not able to click on See Residential Flash Report Link", "Residential Flash Report Link");
		}
	}
	...
	public void selectHistoricalOption() throws IOException {
		...
		{
			Report.FailTestSnapshot(test, driver, "Not able to Select Historical Option", "Historical Option");
		}
	}
	...
	public void selectDetailSubView() throws IOException {
		...
		{
			Report.FailTestSnapshot(test, driver, "Not able to Select Detail Sub View", "Detail Sub View");
		}
	}
	...
	public void validateSummaryData(Map<String, List<String>> actualSummaryTableData,
		...
		validatePlanEfficiency(actualSummaryTableData, actualDetailTableData);
		...
		validateTargetEfficiency(actualSummaryTableData, actualEfficiencyTableData);
		validateSummaryPlanDriverHours(actualSummaryTableData, actualEfficiencyTableData);
		validateActualDriverHours(actualSummaryTableData, actualEfficiencyTableData);
		validateDriverHourVariance(actualSummaryTableData);
	...
	public void validatePlanEfficiency(Map<String, List<String>> actualSummaryTableData,
		...
		try {
			...
			if (actualPlanEfficiency == expectedPlanEfficiency) {
	...
	public void validateSummaryActualEfficiency(Map<String, List<String>> actualSummaryTableData,
		...
		try {
			...
			if (actualEfficiency == expectedActualEfficiency) {
	...
	public void validateTargetEfficiency(Map<String, List<String>> actualSummaryTableData,
		...
		try {
			...
			if (actualTargetEff == expectedTargetEff) {
	...
	public void validateSummaryPlanDriverHours(Map<String, List<String>> actualSummaryTableData,
		...
		try {
			...
			if (Util.compareTime(actualPlanDriverHours, expectedPlanDriverHours))
	...
	public void validateActualDriverHours(Map<String, List<String>> actualSummaryTableData,
			Map<String, List<String>> actualEfficiencyTableData) {
	...
	public void validateDriverHourVariance(Map<String, List<String>> actualSummaryTableData) {
		...
		try {
			...
			if (!(planDriverHours.equals("00:00") && actualDriverHours.equals("00:00"))) {
				...
				if (diffHours < 0 || diffMinutes < 0) {
					...
					if (diffHours > 9 && diffMinutes < 10)
						expectedDriverVariance = Long.toString(diffHours) + ":" + "0" + Long.toString(diffMinutes);
					else if (diffHours < 10 && diffMinutes < 10)
						expectedDriverVariance = "0" + Long.toString(diffHours) + ":" + "0"
	...
	public void validateLOB(Map<String, List<String>> actualEfficiencyTableData, String expectedLOB) {
		...
		try {
			...
			if (actualLOB.equals(expectedLOB)) {
				Report.InfoTest(test, "LOB is correct in Flash report Actual is : " + actualLOB + " and Expected is : "
						+ expectedLOB);
				Report.PassTest(test, "LOB is as expected in Flash report");
			} else {
				Report.FailTest(test, "LOB is not as expected in Flash report Actual is : " + actualLOB
	...
	public void validateEfficiencyClockInTime(Map<String, List<String>> actualEfficiencyTableData,
		...
		try {
			...
			if (actualClockInTime.contains(expectedClockInTime)) {
				...
				Report.FailTest(test, "Clock In Time is not as expected in summary report Actual List is : "
						+ actualClockInTime.toString() + " and Expected is : " + expectedClockInTime);
			}

		} catch (Exception e) {
			Report.FailTest(test, "Clock In Time is not as expected in summary report Actual is : "
					+ actualClockInTime.toString() + " and Expected is : " + expectedClockInTime);
		}
	...
	public void validateEfficiencyClockOutTime(Map<String, List<String>> actualEfficiencyTableData,
		...
		try {
			...
			if (actualClockOutTime.contains(expectedClockOutTime)) {
				...
				Report.FailTest(test, "Clock Out Time is not as expected in summary report Actual List is : "
						+ actualClockOutTime.toString() + " and Expected is : " + expectedClockOutTime);
			}

		} catch (Exception e) {
			Report.FailTest(test, "Clock Out Time is not as expected in summary report Actual is : "
					+ actualClockOutTime.toString() + " and Expected is : " + expectedClockOutTime);
		}
	...
	public void validateRouteActualEfficiency(Map<String, List<String>> actualEfficiencyTableData,
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualEfficiency.get(i) == expectedActualEfficiency) {
					Report.InfoTest(test,
					...
					Report.PassTest(test, "Actual Efficiency is as expected in Residential Efficiency report");
				} else {
					Report.FailTest(test,
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualDriverHourVariance.get(i).equals(expectedDriverHourVariance)) {
					Report.InfoTest(test,
							"Driver Hour variance is correct in efficiency report for Route : " + route.get(i)
					...
					Report.PassTest(test, "Driver Hour Variance is as expected in Residential Efficiency report");
				} else {
					Report.FailTest(test,
							"Driver Hour Variance is not as expected in efficiency report for Route : " + route.get(i)
									+ " with Actual Driver Hour Variance : " + actualDriverHourVariance.get(i)
									+ " and expected Driver Hour Variance : " + expectedDriverHourVariance);
				}
			}
	...
	public void validateEfficiencyVariance(Map<String, List<String>> actualEfficiencyTableData) {
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualEffVariance.get(i) == expectedEffVariance) {
					Report.InfoTest(test,
					...
					Report.PassTest(test, "Units Variance is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
	...
	public void validateDetailPreRouteTime(Map<String, List<String>> actualEfficiencyTableData,
		...
		try {
			...
			leaveYardTime = Util.getDateDataFromDB(
					"select TO_CHAR(FROMYARDSTAMP - (5 / 24) , 'HH:MI') from OCS_ADMIN.TP_RO_RESULT where FK_ROUTEORDER IN (select PKEY from OCS_ADMIN.TP_ROUTEORDER where FK_ROUTE IN (select PKEY from OCS_ADMIN.TP_ROUTE where ID  IN  ("
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualPreRouteTime.get(i).equals(expectedPreRouteTime)) {
					...
					Report.PassTest(test, "Pre-Route time is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
	...
	public void validateDetailUnitVariance(Map<String, List<String>> actualDetailTableData) {
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualUnitsVariance.get(i) == expectedUnitsVariance) {
					Report.InfoTest(test,
				...
				if (actualUnits.get(i).equals(expectedUnits.get(i))) {
					Report.InfoTest(test,
							"Actual Units is correct in detail report for Route : " + route.get(i)
									...
									+ expectedUnits.get(i));
					Report.PassTest(test, "Actual Units is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
				...
				if (actualLunchTime.get(i).equals(expectedLunchTime)) {
					Report.InfoTest(test,
							"Meal Time is correct in detail report for Route : " + route.get(i)
									+ " with Actual Meal Time : " + dateFormat.format(actualLunchTime.get(i)) + " and expected Meal Time : "
									+ dateFormat.format(expectedLunchTime));
					Report.PassTest(test, "Meal Time is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualDownTime.get(i).equals(expectedDownTime)) {
					...
					Report.PassTest(test, "Downtime is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualIdleTime.get(i).equals(expectedIdleTime)) {
					...
					Report.PassTest(test, "Net Idle Time is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualPostRouteTime.get(i).equals(expectedPostRouteTime)) {
					...
					Report.PassTest(test, "Post-Route time is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
	...
	public void validateDetailDisposalCycleTime(Map<String, List<String>> actualDetailTableData) {
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (arriveDisposalTime != null && departDisposalTime != null) {
					...
					if (actualDisposalCycleTime.get(i).equals(expectedDisposalTime)) {
						...
						Report.PassTest(test, "Disposal Cycle Time is as expected in Residential Detail report");
					} else {
						Report.FailTest(test,
			...
			Report.FailTest(test, "Disposal cycle Time is not as expected in Detail report Actual is : "
					+ actualDisposalCycleTime.toString() + " and Expected is : " + expectedDisposalTime);
		}
		...
		try {
			...
			for (int i = 0; i < route.size(); i++) {
				...
				if (actualOnRouteTime.get(i).equals(expectedOnRouteTime)) {
					...
					Report.PassTest(test, "On-Route time is as expected in Residential Detail report");
				} else {
					Report.FailTest(test,
	...
	public void validateEfficiencyPlanDriverHours(Map<String, List<String>> actualEfficiencyTableData,Map<String, List<String>> actualDetailTableData) {
		List<Double> actualUnits=new ArrayList<>();
				...
				if (actualPlanDriverHours.get(i).equals(expectedPlanDriverHours)) {
					Report.InfoTest(test,
							"Plan Driver Hours is correct in efficiency report for Route : " + route.get(i)
									+ " with Actual Plan Driver Hour : " + dateFormat.format(actualPlanDriverHours.get(i))
									+ " and expected Plan Driver Hour : " + dateFormat.format(expectedPlanDriverHours));
					Report.PassTest(test, "Plan Driver Hour is as expected in Residential Efficiency report");
				} else {
					Report.FailTest(test,
							"Plan Driver Hour is not as expected in efficiency report for Route : " + route.get(i)
									+ " with Actual Plan Driver Hour : " + dateFormat.format(actualPlanDriverHours.get(i))
									+ " and expected Driver Hour Variance : " + dateFormat.format(expectedPlanDriverHours));
				}
	...
	public void validateColumns(List<String> actualColumnNames, List<String> expectedColumnNames, String tableName) {
		...
		for (int i = 0; i < expectedColumnNames.size(); i++) {
			if (expectedColumnNames.get(i).equals(expectedColumnNames.get(i))) {
				...
				Report.PassTest(test, "Column Names are as expected in Residential Total section of the report");
			} else {
	...
	public void validatePlanUnits(Map<String, List<String>> actualDetailTableData,String routeEMAPStatus)
	{
		...
		try{
			...
			if(routeEMAPStatus.equals("Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (!planUnits.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Units is as per EMAP in Residential Flash report for Planned Route : " + route.get(i)
										+ " with Plan Units : " + planUnits.get(i));
						Report.PassTest(test, "Plan Units is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Units is not as expected in detail report for Planned Route : " + route.get(i)
										+ " with Plan Units : " + planUnits.get(i));
					}
				}
			...
			else if(routeEMAPStatus.equals("Un-Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (planUnits.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Units is as per EMAP in Residential Flash report for Un-Planned Route : " + route.get(i)
										+ " with Plan Units : " + planUnits.get(i));
						Report.PassTest(test, "Plan Units is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Units is not as expected in detail report for Un-Planned Route : " + route.get(i)
										+ " with Plan Units : " + planUnits.get(i));
					}
				}
			}
			...
			{
				Report.FailTest(test, "Route status should be either Planned or Un-Planned. Please check method calling.");
			}
		...
		catch(Exception e)
		{
			...
			Report.FailTest(test, "Plan Units is not as expected in Residential Flash report Actual is : "
					+ planUnits.toString());
		}
	...
	public void validatePlanDriverHours(Map<String, List<String>> actualEfficiencyTableData,String routeEMAPStatus)
	{
		...
		try{
			if(routeEMAPStatus.equals("Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (!planDriverHours.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Driver Hours is as per EMAP in Residential Flash report for Planned Route : " + route.get(i)
										+ " with Plan Driver Hours : " + planDriverHours.get(i));
						Report.PassTest(test, "Plan Driver Hours is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Driver Hours is not as expected in efficiency report for Planned Route : " + route.get(i)
										+ " with Plan Driver Hours : " + planDriverHours.get(i));
					}
				}
			...
			else if(routeEMAPStatus.equals("Un-Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (planDriverHours.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Driver Hours is as per EMAP in Residential Flash report for Un-Planned Route : " + route.get(i)
										+ " with Plan Driver Hours : " + planDriverHours.get(i));
						Report.PassTest(test, "Plan Driver Hours is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Driver Hours is not as expected in efficiency report for Un-Planned Route : " + route.get(i)
										+ " with Plan Driver Hours : " + planDriverHours.get(i));
					}
				}
			}
		...
		{
			Report.FailTest(test, e.getMessage());
			Report.FailTest(test, "Plan Driver Hours is not as expected in Residential Flash report Actual is : "
	...
	public void validatePlanEfficiency(Map<String, List<String>> actualEfficiencyTableData,String routeEMAPStatus)
	{
		...
		try{
			if(routeEMAPStatus.equals("Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (!planEfficiency.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Efficiency is as per EMAP in Residential Flash report for Planned Route : " + route.get(i)
										+ " with Plan Efficiency : " + planEfficiency.get(i));
						Report.PassTest(test, "Plan Efficiency is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Efficiency is not as expected in efficiency report for Planned Route : " + route.get(i)
										+ " with Plan Efficiency : " + planEfficiency.get(i));
					}
				}
			...
			else if(routeEMAPStatus.equals("Un-Planned"))
			{
				...
				for (int i = 0; i < route.size(); i++) {
					if (planEfficiency.get(i).equals("--")) {
						Report.InfoTest(test,
								"Plan Efficiency is as per EMAP in Residential Flash report for Un-Planned Route : " + route.get(i)
										+ " with Plan Efficiency : " + planEfficiency.get(i));
						Report.PassTest(test, "Plan Efficiency is as expected in Residential Flash report");
					} else {
						Report.FailTest(test,
								"Plan Efficiency is not as expected in detail report for Un-Planned Route : " + route.get(i)
										+ " with Plan Efficiency : " + planEfficiency.get(i));
					}
				}
			}
		...
		{
			Report.FailTest(test, e.getMessage());
			Report.FailTest(test, "Plan Efficiency is not as expected in Residential Flash report Actual is : "
...
public void validateLegendInGraph() throws IOException, InterruptedException {
	...
	public void verifyPDF_ColumnNames(String downloadFilePath,List <String> ListOfColumns)
	{
		//String[] columnName = ListOfColumns.split(";");
		try
		{
			clickPDFLink();
			Util.switchBrowserTabsAndStay();
			Util.keystrokes_ControlS();
			Runtime.getRuntime().exec(System.getProperty("user.dir")+"\\AutoIt\\ChromeDownload_FlashReport.exe");
			Thread.sleep(15000);
			File recentFile=Util.getLatestFilefromDir("C:\\Working\\PDFDownloads");
			System.out.println(recentFile.getName());
					
			verifyPDFContent("C:\\Working\\PDFDownloads"+"\\"+recentFile.getName(),ListOfColumns);
			driver.close();
	...
	public void verifyPDFContent(String filepath,List<String> reqTextInPDF)
	{
		for (int i = 0; i < reqTextInPDF.size(); i++) {
			try {
				String allText = Util.getPDFText(filepath);
				//System.out.println(allText);
				if (allText.contains(reqTextInPDF.get(i))) {
					System.out.println(reqTextInPDF.get(i) + " is present in the PDF");
					Report.PassTest(test, reqTextInPDF.get(i) + " is present in the PDF");
				}
	...
	public void clickPDFLink() throws IOException
	{	
		Util.switchToDefaultWindow();
		Util.pageScroll(0, 40);
		Util.selectFrame("opusReportContainer,subReportContainer");
		if(PDFLink.isDisplayed())
		{
			PDFLink.click();
			Report.PassTest(test, "PDF was clicked");
		}
		else 
		{
			System.out.println("PDF link was not clicked");
			Report.FailTest(test, "PDF Link was not clicked");
		}
