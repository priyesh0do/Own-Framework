...
public class Data_Validation_For_DNR_Stopped_Time extends BaseClass {
	...
	public void DataValidation_For_RO_TicketType_DNR(String ReportName, String siteID, String LOB, String FromDate,
			String ToDate, String Route, String Driver,String Ticket,String TicketType) throws Exception {	
		if (!Util.isTestCaseExecutable("Data_Validation_For_DNR_Stopped_Time",GlobalVariables.sdoTestCase))
		{
			test = Report.testCreate(extent, test, "Data_Validation_For_DNR_Stopped_Time");
			Report.SkipTest(test, "Skipping the test as runmode is NO");			
		}
		else
		{
			if(TicketType.equalsIgnoreCase("DNR"))
			{
			    test = Report.testCreate(extent, test, "Data_Validation_For_DNR_Stopped_Time");
