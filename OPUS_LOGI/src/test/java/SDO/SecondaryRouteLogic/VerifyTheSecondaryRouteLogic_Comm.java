...
public class VerifyTheSecondaryRouteLogic_Comm extends BaseClass{
	 ...
	 * 
	 * Verify Plan Units, Pan Driver Hours and Plan Efficiency are populating from EMAP in commercial flash report
	 * 
	 */
	...
	public void OpusSDO_SecondaryRouteLogicCommercialFlashReport(String ReportName, String siteID, String LOB, String FromDate,
		...
		if (!Util.isTestCaseExecutable("Verify the Secondary Route Logic_Comm",GlobalVariables.sdoTestCase))
		{
			test = Report.testCreate(extent, test, "Verify the Secondary Route Logic_Comm");
		...
		else
		{
			...
			sdoCommObj.validatePlanUnits(actualDetailTable,"Planned");
			sdoCommObj.validatePlanDriverHours(actualEfficiencyTable,"Planned");
			sdoCommObj.validatePlanEfficiency(actualEfficiencyTable,"Planned");
			sdoCommObj.validateEfficiencyPlanDriverHours(actualEfficiencyTable, actualDetailTable);
            driver.quit();
		}
